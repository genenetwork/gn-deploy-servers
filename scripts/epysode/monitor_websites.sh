...
TAG=WEB_BNW
URL="https://bnw.genenetwork.org/sourcecodes/home.php"
FIND=BNW
echo $TAG
sheepdog_run.rb -c "curl $URL|grep $FIND" --tag $TAG $*
...
TAG=WEB_RATS_PUB
URL="--connect-timeout 30 --retry 3 --retry-delay 5 https://rats.pub/"
FIND=Addiction
echo $TAG
sheepdog_run.rb -c "curl $URL|grep $FIND" --tag $TAG $*

TAG=WEB_PUBSEQ
URL=https://covid19.genenetwork.org/
FIND=PubSeq
echo $TAG
sheepdog_run.rb -c "curl $URL|grep -i $FIND" --tag $TAG $*
